  ...
  <div>
    <table class="d-rtl">
      <tr class="table_header">
        <th>{{ getTextByTextKey("profile_order_id") }}</th>
        <th>{{ getTextByTextKey("profile_order_date") }}</th>
        <th>{{ getTextByTextKey("profile_order_total_amount") }}</th>
        <th>{{ getTextByTextKey("profile_order_status") }}</th>
        <th>{{ getTextByTextKey("profile_order_detail") }}</th>
      </tr>
        ...
        <td class="user-listOrders__empty-holder">
          <div class="user-listOrders__empty-container">
            <span class="user-listOrders__empty d-rtl">
              {{ getTextByTextKey("profile_not_found") }}
            </span>
          </div>
        </td>
        <td></td>
      ...
      <tr v-for="data in tableData" :key="data.id" class="table_content">
        ...
        <td>{{ data.orderPrice }} {{ getTextByTextKey("public_unit") }}</td>
        <td>{{ data.orderSituation }}</td>
        <td>
          <NuxtLink
            :to="'/profile/my-orders/in-progress/detail/' + data.orderCode"
            class="p-order-list-btn-link"
          >
            <span class="btn-text__order-detail">
